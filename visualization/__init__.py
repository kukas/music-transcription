...
def draw_notes(ref, est, style=".", title=None):
    ...
    indices_correct, correct = flatten([[n_est for n_est in fest if any([abs_correct(n_ref, n_est) for n_ref in fref])] for fref, fest in zip(ref, est)])
    indices_unvoiced_incorrect, unvoiced_incorrect = flatten([[n_est for n_est in fest if len(fref) == 0] for fref, fest in zip(ref, est)])
    indices_incorrect_chroma, incorrect_chroma = flatten([[n_est for n_est in fest if all([not abs_correct(n_est, n_ref) and octave_correct(n_ref, n_est) for n_ref in fref]) and len(fref) > 0] for fref, fest in zip(ref, est)])
    indices_incorrect, incorrect = flatten([[n_est for n_est in fest if all([not abs_correct(n_ref, n_est) and not octave_correct(n_ref, n_est) for n_ref in fref]) and len(fref) > 0] for fref, fest in zip(ref, est)])
